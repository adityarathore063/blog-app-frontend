...
const BlogDetail = () => {
  ...
  const fetchDetails = async () => {
    const res = await axios.get(`https://blog-app-api-s3xz.onrender.com/api/blog/${id}`).catch(err => console.log(err));
  ...
  const sendRequest = async () => {
    const res = await axios.put(`https://blog-app-api-s3xz.onrender.com/api/blog/update/${id}`,{
      title:inputs.title,
